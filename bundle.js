(function (React$1, ReactDOM$1, d3$1, ReactDropdown) {
  ...
  function transformData(data, col) {
    var transformed = d3$1.nest()
      ...
      .rollup(function (d) {
        return {
          Amount: d.length,
          "Average Age": d3$1.mean(
            d.map(function (correspondent) { return correspondent["Average Age"]; })
          ),
          "Average Time Served": d3$1.mean(
          ...
          tooltip
            .html(
              ("<div>" + (toTitle(xAttribute)) + ": " + (d.key) + "</div>\n              <div>" + (toTitle(yAttribute)) + ": " + (formatNumber(
                d.value[yAttribute].toFixed(2)
              )) + "</div>\n              <div>" + ("Percent") + ": " + (formatNumber(
                ((d.value[yAttribute] / totalPopulation) * 100).toFixed(2)
              )) + "%</div>\n              <div>There are " + (formatNumber(d.value.younger)) + " people " + (d.key) + " or younger under custody (" + (formatNumber(
                ((d.value.younger / totalPopulation) * 100).toFixed(1)
              )) + "%)</div>\n              <div>There are " + (formatNumber(d.value.older)) + " people over " + (d.key) + " under custody (" + (formatNumber(
                ((d.value.older / totalPopulation) * 100).toFixed(1)
              )) + "%)</div>")
            )
  ...
  var Chart = function (ref) {
    ...
    var ref$1 = React$1.useState("Gender");
